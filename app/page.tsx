...
const Card = ({ children, ...props }: { children: ReactNode; [key: string]: any }) => (
  <div className="rounded-xl border p-4 shadow">{children}</div>
);
const CardContent = ({ children, ...props }: { children: ReactNode; [key: string]: any }) => (
...
export default function ZeitschaetzlaufApp() {
  ...
  function calcDeviation(ziel: number, ist: number): number {
  ...
  function scoreDeviation(pct: number): number {
    if (pct <= 1) return 10;
    if (pct <= 2) return 9;
    if (pct <= 3) return 8;
    if (pct <= 4) return 7;
  ...
  function scoreConsistency(std: number): number {
    if (std <= 2) return 5;
    if (std <= 4) return 4;
    if (std <= 6) return 3;
    if (std <= 8) return 2;
